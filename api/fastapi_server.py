...
@app.get("/api/v1/customers/{customer_id}/usage",
...
async def get_customer_usage(
    ...
    cache_key = cache.generate_cache_key("customer_usage", customer_id, 
    ...
    result = await db.fetchrow(query, customer_id, date_range.start_date, date_range.end_date)
...
@app.get("/api/v1/network/performance",
...
async def get_network_performance(
    ...
    cache_key = cache.generate_cache_key("network_performance", region, technology, min_health_score)
    cached = await cache.get(cache_key)
