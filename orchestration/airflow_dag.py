...
def check_data_freshness(**context):
    ...
    expected_file = f"s3://telestream-raw/cdr/{execution_date.strftime('%Y-%m-%d')}/cdr_data.csv"
    
    # Check if file exists and was created within last 2 hours
    s3 = boto3.client('s3')
    try:
        response = s3.head_object(Bucket='telestream-raw', 
...
def validate_schema(**context):
    ...
    expectations = [
        ge_df.expect_table_columns_to_match_ordered_list([
            'call_id', 'customer_id', 'call_type', 'call_start_time', 
            'duration_seconds', 'data_volume_mb', 'call_cost'
        ]),
    ...
    if failed:
        raise AirflowException(f"Schema validation failed: {json.dumps(failed, indent=2)}")
...
def run_data_quality_checks(**context):
    ...
    execution_date = context['execution_date']
    batch_id = context['task_instance'].xcom_pull(task_ids='transform_bronze_to_silver', key='batch_id')
    ...
    """, parameters=(
        'silver_cdr', checkpoint_name, datetime.now(),
        total_checks, passed_checks, (passed_checks/total_checks)*100, 'PASSED'
    ))
    ...
    **Retry Policy**: 3 retries with exponential backoff
    ...
    with TaskGroup(group_id='gold_aggregation') as gold_aggregation:
        ...
        proceed = BashOperator(
            ...
            bash_command='echo "Proceeding to gold layer"'
        )
