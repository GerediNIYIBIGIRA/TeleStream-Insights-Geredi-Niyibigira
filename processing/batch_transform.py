...
def normalize_columns(df: DataFrame, columns: List[str]) -> DataFrame:
    """Normalize specified columns to lowercase and trim whitespace"""
    result = df
    for col in columns:
        if col in df.columns:
            result = result.withColumn(col, F.trim(F.lower(F.col(col))))
    return result


def enrich_with_hash(
    df: DataFrame, hash_columns: List[str], hash_col_name: str = "row_hash"
) -> DataFrame:
    """Add hash column based on specified columns"""
    return df.withColumn(
        hash_col_name, F.sha2(F.concat_ws("|", *[F.col(c) for c in hash_columns]), 256)
    )
...
def apply_data_quality_filters(df: DataFrame) -> DataFrame:
    ...
    return (
        df.filter(F.col("call_id").isNotNull())
        .filter(F.col("customer_id").isNotNull())
        .filter(F.col("call_start_time").isNotNull())
        .filter(F.col("duration_seconds") >= 0)
        .filter(F.col("data_volume_mb") >= 0)
...
def add_temporal_features(
    df: DataFrame, timestamp_col: str = "call_start_time"
) -> DataFrame:
    """Add temporal features from timestamp column"""
    return (
        df.withColumn("call_date", F.to_date(timestamp_col))
        .withColumn("call_hour", F.hour(timestamp_col))
        .withColumn("call_day_of_week", F.dayofweek(timestamp_col))
        .withColumn(
            "is_weekend",
            F.when(F.col("call_day_of_week").isin([1, 7]), True).otherwise(False),
        )
    )


def add_call_classifications(df: DataFrame) -> DataFrame:
    """Add call classification features"""
    return (
        df.withColumn(
            "call_duration_minutes", F.round(F.col("duration_seconds") / 60, 2)
        )
        .withColumn(
            "is_short_call",
        ...
        .withColumn(
            "is_dropped_call",
            F.when(
                (F.col("duration_seconds") < 30) & (F.col("call_type") == "voice"), True
            ).otherwise(False),
        )
    )


def add_revenue_category(df: DataFrame, cost_col: str = "call_cost") -> DataFrame:
    """Categorize revenue based on call cost"""
    return df.withColumn(
        "revenue_category",
        F.when(F.col(cost_col) > 10, "high")
        .when(F.col(cost_col) > 5, "medium")
        .otherwise("low"),
    )


def add_signal_quality(
    df: DataFrame, signal_col: str = "signal_strength_dbm"
) -> DataFrame:
    """Classify signal quality based on dBm"""
    return df.withColumn(
        "signal_quality",
        F.when(F.col(signal_col) > -70, "excellent")
        .when(F.col(signal_col) > -85, "good")
        .when(F.col(signal_col) > -100, "fair")
        .otherwise("poor"),
    )
...
def enrich_with_customer_segment(df: DataFrame) -> DataFrame:
    """Enrich with customer segment (mock for testing)"""
    return df.withColumn(
        "customer_segment",
        F.when(F.rand() > 0.7, "enterprise")
        .when(F.rand() > 0.4, "consumer")
...
def aggregate_daily_usage(df: DataFrame) -> DataFrame:
    """Aggregate usage metrics by customer and date"""
    return (
        df.groupBy("customer_id", "call_date")
        .agg(
            F.count("call_id").alias("total_calls"),
            F.sum("duration_seconds").alias("total_duration_seconds"),
            F.sum("data_volume_mb").alias("total_data_mb"),
            ...
            F.sum("call_cost").alias("total_cost"),
            F.avg("signal_strength_dbm").alias("avg_signal_strength"),
            F.sum(F.when(F.col("is_dropped_call"), 1).otherwise(0)).alias(
                "dropped_calls_count"
            ),
            F.sum(
                F.when(F.col("roaming_flag"), F.col("duration_seconds")).otherwise(0)
            ).alias("roaming_duration_seconds"),
            F.countDistinct("originating_cell_id").alias("unique_cells_used"),
            F.max("call_start_time").alias("last_call_time"),
            F.first("customer_segment").alias("customer_segment"),
...
def aggregate_network_performance(df: DataFrame) -> DataFrame:
    """Aggregate network performance metrics by cell tower"""
    return (
        df.groupBy("originating_cell_id", "call_date")
        .agg(
            F.count("call_id").alias("total_calls"),
            F.countDistinct("customer_id").alias("unique_customers"),
            F.avg("signal_strength_dbm").alias("avg_signal_strength"),
            ...
            F.expr("percentile_approx(duration_seconds, 0.95)").alias(
                "p95_call_duration"
            ),
        )
        .withColumn(
...
def detect_usage_anomalies(df: DataFrame, lookback_days: int = 30) -> DataFrame:
    ...
    df_with_stats = (
        df.withColumn("avg_data_30d", F.avg("total_data_mb").over(window_spec))
        .withColumn("stddev_data_30d", F.stddev("total_data_mb").over(window_spec))
        .withColumn("avg_cost_30d", F.avg("total_cost").over(window_spec))
        .withColumn("stddev_cost_30d", F.stddev("total_cost").over(window_spec))
    )
...
class TelestreamETLPipeline:
    ...
    def __init__(self, environment: str = "dev"):
        ...
        self.spark = self._create_spark_session()
        self.data_lake_path = (
            "s3a://telestream-datalake" if environment == "prod" else "/tmp/datalake"
        )

    def _create_spark_session(self) -> SparkSession:
        """Create optimized Spark session with Delta Lake"""
        builder = (
            SparkSession.builder.appName(f"TeleStream-ETL-{self.environment}")
            .config("spark.sql.extensions", "io.delta.sql.DeltaSparkSessionExtension")
            .config(
    ...
    def get_cdr_schema(self) -> StructType:
        """Define CDR schema"""
        return StructType(
            [
                StructField("call_id", StringType(), False),
    ...
    def ingest_cdr_to_bronze(self, source_path: str, batch_id: str) -> None:
        """Ingest raw CDR data to Bronze layer"""
        logger.info(f"Starting CDR ingestion for batch {batch_id}")

        df_raw = (
            self.spark.read.option("header", "true")
            .option("mode", "PERMISSIVE")
            .schema(self.get_cdr_schema())
            .csv(source_path)
        )

        # Apply transformations using pure functions
        df_bronze = add_metadata_columns(df_raw, batch_id)
        df_bronze = enrich_with_hash(df_bronze, df_raw.columns)
        ...
        if DeltaTable.isDeltaTable(self.spark, bronze_path):
            ...
            df_bronze.write.format("delta").mode("overwrite").partitionBy(
                "batch_id"
            ).save(bronze_path)
    ...
    def transform_bronze_to_silver(self, batch_date: str) -> None:
        ...
        # Apply transformations using pure functions
        df_silver = apply_data_quality_filters(df_bronze)
        df_silver = add_temporal_features(df_silver)
        df_silver = add_call_classifications(df_silver)
        df_silver = add_revenue_category(df_silver)
        df_silver = add_signal_quality(df_silver)
        df_silver = enrich_with_customer_segment(df_silver)
        df_silver = calculate_data_quality_score(df_silver)

        df_silver = df_silver.withColumn("silver_timestamp", F.current_timestamp())

        df_silver.write.format("delta").mode("overwrite").partitionBy("call_date").save(
            silver_path
        )

        self._log_data_quality_metrics(df_silver, "cdr_silver")
        logger.info("Silver transformation completed")

    def create_gold_daily_usage_summary(self, target_date: str) -> None:
        """Create daily usage aggregates"""
        logger.info(f"Creating Gold layer daily summary for {target_date}")
        ...
        df_gold = aggregate_daily_usage(df_silver)
    ...
    def create_gold_network_performance(self, target_date: str) -> None:
        ...
        df_gold = aggregate_network_performance(df_silver)

        df_gold.write.format("delta").mode("overwrite").partitionBy("call_date").save(
            gold_path
        )

    def detect_anomalies(self, lookback_days: int = 30) -> DataFrame:
        """Real-time anomaly detection"""
        gold_path = f"{self.data_lake_path}/gold/daily_usage_summary"
        df = self.spark.read.format("delta").load(gold_path)
        return detect_usage_anomalies(df, lookback_days)

    def _log_data_quality_metrics(self, df: DataFrame, table_name: str) -> None:
        """Log data quality metrics"""
        total_records = df.count()
        null_counts = {
            col: df.filter(F.col(col).isNull()).count() for col in df.columns[:5]
        }  # Sample first 5 columns

        logger.info(f"Data Quality Metrics for {table_name}: {total_records} records")
