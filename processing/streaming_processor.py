...
class RealtimeNetworkMonitoring:
    ...
    def _create_spark_session(self) -> SparkSession:
        ...
        builder = (
            SparkSession.builder.appName("TeleStream-RealtimeMonitoring")
    ...
    def read_network_events_stream(self) -> DataFrame:
        ...
        df_enriched = (
            df_parsed.withColumn("processing_time", F.current_timestamp())
            .withColumn(
    ...
    def compute_cell_metrics_windowed(self, df_stream: DataFrame) -> DataFrame:
        ...
        df_windowed = (
            df_stream.groupBy(
                F.window("event_timestamp", "5 minutes", "1 minute"), "cell_id"
            )
            .agg(
                ...
                F.sum(
                    F.when(F.col("event_type") == "congestion", 1).otherwise(0)
                ).alias("congestion_events"),
                F.sum(F.when(F.col("event_type") == "outage", 1).otherwise(0)).alias(
                    "outage_events"
                ),
                F.sum(F.when(F.col("error_rate_percent") > 5, 1).otherwise(0)).alias(
                    "high_error_events"
                ),
            )
            .select(
        ...
        df_scored = df_windowed.withColumn(
            "health_score",
            F.greatest(
                F.lit(0),
                F.lit(100)
    ...
    def detect_realtime_anomalies(self, df_stream: DataFrame) -> DataFrame:
        ...
        df_stats = df_stream.groupBy(window_spec_30m, "cell_id").agg(
        ...
        df_with_stats = df_stream.alias("current").join(
    ...
    def track_cell_state(self, df_stream: DataFrame) -> DataFrame:
        ...
        def update_cell_state(cell_id, events, state):
            ...
            if state.exists:
                ...
                cumulative_errors = old_state["cumulative_errors"] + sum(
                    e["error_rate_percent"] for e in events
                )
            else:
        ...
        df_stateful = df_stream.groupByKey(lambda row: row.cell_id).mapGroupsWithState(
            update_cell_state, outputMode="update", timeoutConf="ProcessingTimeTimeout"
        )
    ...
    def write_to_delta_lake(
        self, df_stream: DataFrame, table_name: str
    ) -> DataStreamWriter:
    ...
    def write_to_kafka(self, df_stream: DataFrame, topic: str) -> DataStreamWriter:
        ...
        df_json = df_stream.select(
            F.col("cell_id").alias("key"), F.to_json(F.struct("*")).alias("value")
        )

        query = (
            df_json.writeStream.format("kafka")
            .option("kafka.bootstrap.servers", self.kafka_brokers)
            .option("topic", topic)
    ...
    def write_to_console(self, df_stream: DataFrame) -> DataStreamWriter:
        ...
        query = (
            df_stream.writeStream.outputMode("update")
            .format("console")
        ...
        def write_batch_to_postgres(batch_df: DataFrame, batch_id: int):
            ...
            if batch_df.count() > 0:
                batch_df.write.format("jdbc").option(
                    "url", "jdbc:postgresql://postgres:5432/telestream_dw"
                ).option("dbtable", table_name).option("user", "telestream").option(
                ...
                logger.info(
                    f"Batch {batch_id}: Written {batch_df.count()} records to {table_name}"
                )

        query = (
            df_stream.writeStream.foreachBatch(write_batch_to_postgres)
            .option(
                "checkpointLocation",
                f"{self.checkpoint_location}/postgres_{table_name}",
            )
    ...
    def generate_alerts(self, df_anomalies: DataFrame) -> DataStreamWriter:
        ...
        query = (
            df_anomalies.writeStream.foreachBatch(send_alert)
