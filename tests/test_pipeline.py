...
def sample_cdr_data():
    ...
    return pd.DataFrame({
        ...
        'call_start_time': [(datetime.now() - timedelta(hours=i)) for i in range(1000)],
        'call_end_time': [(datetime.now() - timedelta(hours=i-1)) for i in range(1000)],
...
class TestDataTransformations:
    ...
    def test_aggregation_logic(self, spark_session, sample_cdr_data):
        ...
        expected_calls = len(sample_cdr_data[sample_cdr_data['customer_id'] == 'CUST-0001'])
...
class TestPipelineIntegration:
    ...
    def test_bronze_to_silver_pipeline(self, spark_session, tmp_path, sample_cdr_data):
        ...
        df_silver.write.format("parquet").mode("overwrite") \
            .partitionBy("call_date").save(silver_path)
    ...
    def test_gold_layer_aggregation(self, spark_session, tmp_path, sample_cdr_data):
        ...
        df_gold.write.format("parquet").mode("overwrite") \
            .partitionBy("call_date").save(gold_path)
        
        # Verify Gold layer
        df_verify = spark_session.read.parquet(gold_path)
        assert df_verify.count() > 0
        assert all(col in df_verify.columns for col in ['customer_id', 'total_calls', 'total_data'])
...
class TestStreamingPipeline:
    ...
    def test_stream_windowing(self, spark_session):
        ...
        df = spark_session.createDataFrame(
            data,
            ['cell_id', 'event_timestamp', 'bandwidth_util', 'error_rate']
        )
...
class TestAPIEndpoints:
    ...
    async def test_customer_usage_endpoint(self, api_client):
        ...
        response = await api_client.get(
            "/api/v1/customers/CUST-0001/usage",
            headers=headers,
            params=params
        )
...
class TestPerformance:
    ...
    def test_large_dataset_processing(self, spark_session):
        ...
        df_large = spark_session.range(0, 1000000).select(
            ...
            F.concat(F.lit('CUST-'), (F.col('id') % 10000).cast('string')).alias('customer_id'),
        ...
        result = df_large.groupBy('customer_id').agg(
            F.count('call_id').alias('total_calls'),
            F.sum('data_volume_mb').alias('total_data')
        ).collect()
...
class TestDataQuality:
    ...
    def test_completeness_check(self, spark_session, sample_cdr_data):
        ...
        for field in critical_fields:
            ...
            assert completeness_rate == 100.0, f"{field} completeness: {completeness_rate}%"
    ...
    def test_consistency_check(self, spark_session, sample_cdr_data):
        ...
        from pyspark.sql import functions as F
        inconsistent = df.filter(F.col('call_end_time') < F.col('call_start_time')).count()
    ...
    def test_accuracy_check(self, spark_session, sample_cdr_data):
        ...
        invalid_signal = df.filter(
            (F.col('signal_strength_dbm') < -120) | 
            (F.col('signal_strength_dbm') > -30)
        ).count()
...
class TestSchemaEvolution:
    ...
    def test_backward_compatibility(self, spark_session, sample_cdr_data):
        ...
        df_compatible = df_old \
            .withColumn('duration_seconds', F.lit(0)) \
            .withColumn('data_volume_mb', F.lit(0.0))
...
class TestErrorHandling:
    ...
    async def test_api_error_responses(self, api_client):
        ...
        response = await api_client.get(
            ...
            headers=headers,
            params={"start_date": "2025-01-31", "end_date": "2025-01-01"}  # end before start
        )
